...
Questions and categories are fetched from the [Open Trivia DB API](https://opentdb.com/):
...
- Open Trivia DB rate-limits requests to roughly one per 5 seconds per IP — rapid consecutive requests (including React StrictMode's double-invoked effects in development) can return a `429` response.
